...
catalogRoutes.put("/destinations/:id", requireStaff, async (c) => {
	const body = await c.req.json();
	const result = await db.update(destinations).set(body).where(eq(destinations.id, c.req.param("id"))).returning();
	return c.json({ destination: result[0] });
});
...
catalogRoutes.post("/universities", requireStaff, async (c) => {
	const body = await c.req.json();
	const result = await db.insert(catalogUniversities).values(body).returning();
	return c.json({ university: result[0] });
});
catalogRoutes.put("/universities/:id", requireStaff, async (c) => {
	const body = await c.req.json();
	const result = await db.update(catalogUniversities).set(body).where(eq(catalogUniversities.id, c.req.param("id"))).returning();
	return c.json({ university: result[0] });
});
...
catalogRoutes.post("/programs", requireStaff, async (c) => {
	const body = await c.req.json();
	const result = await db.insert(catalogPrograms).values(body).returning();
	return c.json({ program: result[0] });
});
catalogRoutes.put("/programs/:id", requireStaff, async (c) => {
	const body = await c.req.json();
	const result = await db.update(catalogPrograms).set(body).where(eq(catalogPrograms.id, c.req.param("id"))).returning();
	return c.json({ program: result[0] });
});
...
catalogRoutes.post("/scholarships", requireStaff, async (c) => {
	const body = await c.req.json();
	const result = await db.insert(catalogScholarships).values(body).returning();
	return c.json({ scholarship: result[0] });
});
catalogRoutes.put("/scholarships/:id", requireStaff, async (c) => {
	const body = await c.req.json();
	const result = await db.update(catalogScholarships).set(body).where(eq(catalogScholarships.id, c.req.param("id"))).returning();
	return c.json({ scholarship: result[0] });
});
