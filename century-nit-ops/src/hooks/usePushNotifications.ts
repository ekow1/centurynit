 ...
 *
 * The console is served same-origin as `/api/*` (the Worker proxies it), so the
 * push subscribe / unsubscribe calls are relative and the Better Auth session
 * cookie travels with them the same way every other `apiFetch` call does. Staff
 * subscriptions land in the same `push_subscriptions` table as clients' — the
 * notification service keys delivery off the recipient's user id, not the app.
 *
 ...
 * Instead this hook only auto-subscribes on mount if the staff member
 * previously granted permission (e.g. they return to the console). Call
 * `subscribe()` explicitly from a control when you want to ask the first time.
 */
