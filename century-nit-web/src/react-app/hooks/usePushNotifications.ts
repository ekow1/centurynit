...
/**
 * Web Push subscription for the client portal.
 *
 * The portal is served same-origin as `/api/*` (the Worker proxies it), so the
 * push subscribe / unsubscribe calls are relative and the Better Auth session
 * cookie travels with them the same way every other `apiFetch` call does.
 *
 * The permission prompt is NEVER shown automatically — browsers throttle
 * `Notification.requestPermission()` and surfacing it on load is a hostile UX.
 * Instead this hook only auto-subscribes on mount if the user previously
 * granted permission (e.g. they return to the site). Call `subscribe()`
 * explicitly from a button when you want to ask the first time.
 */
...
export type UsePushNotificationsOptions = {
	/** Only subscribe while the user is signed in. */
...
export function usePushNotifications(
	...
	/**
	 * Auto-subscribe on mount (and when auth flips on) ONLY if the user has
	 * already granted notification permission. This handles the "returning
